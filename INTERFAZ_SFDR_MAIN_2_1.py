...
class MainWindow (QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.valoresdemuestras=None
                # Crear instancias de Canvas y Toolbar
        ...
        #self.Cancelar.clicked.connect(self.canc)
    ...
    def enviarajustes(self):
        ...
        if (len(self.arreglodeprng*8)==self.cantmuestras):
            ...
            self.puerto_serie.write(bytes.fromhex('23 53 44'))# van las muestras
            self.puerto_serie.write(cantidaddemuestras.to_bytes(1, byteorder='big') ) 
            self.puerto_serie.write(bytes.fromhex('01'))# relleno la frecuencia que antes mandaba)
            self.puerto_serie.write(self.arreglodeprng)
             #print("hasta aca llegamos 1")
            self.label_6.setText("Parametros enviados")
            receptor=self.puerto_serie.read(size=1)
            # print(receptor)
            if int.from_bytes(receptor, byteorder='big') == 0x11: # en hexa 11
    ...
    def guardartimer(self):
        ...
        for i in range(0, doblecantidaddemuestras):
            lectura[i]=int.from_bytes(self.puerto_serie.read(size=1),byteorder='little')# cantidad de bytes a leer.Byteorder te dice si el primer valor a leer es el mas grande o el menor
   ...
   #grafico y calculo
    def grafico(self):
        ...
        #cambio frec de muestreo y cantidad de muestras por el diezmado
        x=self.valoresdemuestras
        #window = np.kaiser(len(t),38)
        #windowed_signal = x * window
        #ffty = np.fft.fft(x)
        #self.frecuencias = np.fft.fftfreq(len(t), TS)
        #self.Magnitude = (np.abs(ffty))/len(t)
        #self.graficafrecuencia.plot(self.frecuencias,self.Magnitude)
...
"  border: none;")
        ...
        #Calculo sfdr
    def calculo(self):
        #sfdr_value,findice,spurioindice, spur_power = sfdr.sfdr_signal(self.valoresdemuestras, 60*1e6//(self.numdiezmado))
        ...
        # para grafico y sfdr a partir de power spectrum
        x=signal_no_dc
        Fs=60000000
        N = len(x)
        f, pxx =utils.periodogram(signal_no_dc, Fs//(self.numdiezmado), window=('kaiser', 38),method="welch", scaling="spectrum")
        Magnitude=10*np.log10(pxx)

        spurioamp=Magnitude[spur_idx]
        fundamental_index = np.argmax(Magnitude)  
        fundamental_power_dB = Magnitude[fundamental_index]
