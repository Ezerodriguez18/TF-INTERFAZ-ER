...
class Canvas_graficafrecuencia(FigureCanvas):
    ...
    def plot(self,frecuencias,magnitude,findice,famp,spurioindice,spurioamp,diezmado,longitud,sfdrvalue):
        self.ax.clear()
        self.ax.grid(True)
        self.ax.margins(x=0)
        ...
        self.ax.ticklabel_format(axis='x', style='sci', scilimits=(0, 0))
        self.ax.xaxis.get_offset_text().set_fontsize(10)  # Tamaño del texto "x10^"
        plt.fill_between((0,60000000/(diezmado*2)),(spurioamp,spurioamp),(famp,famp), label=f'SFDR = {sfdrvalue:.2f} dB',
                     color = "lightblue")  
        self.ax.plot(findice, famp, marker="s", label=f'fundamental =  {famp:.2f} dB')
        self.ax.plot(spurioindice, spurioamp, marker="s",label =f'spurs =  {spurioamp:.2f} dB')
